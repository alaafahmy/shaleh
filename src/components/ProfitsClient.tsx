...
type Payment = { amount: number; date: Date };
type Expense = { amount: number; date: Date };
type Reservation = { totalCost: number; payments: { amount: number }[] };

export default function ProfitsClient({
  payments,
  expenses,
  reservations,
  canExport,
  ...
  const filteredPayments = useMemo(() => 
    payments.filter(p => new Date(p.date) >= filterDateRange.start && new Date(p.date) < filterDateRange.end),
  [payments, filterDateRange]);

  const filteredExpenses = useMemo(() => 
    expenses.filter(e => new Date(e.date) >= filterDateRange.start && new Date(e.date) < filterDateRange.end),
  ...
  // ── حساب الأرباح الشهرية الحقيقية (للسنة الحالية) ──
  const now = new Date();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex bg-[var(--color-ui-bg-panel)] p-1 rounded-xl shadow-lg shadow-[var(--color-brand-glow)] overflow-x-auto w-full md:w-auto">
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-all duration-300 flex-1 whitespace-nowrap text-sm ${
                activeTab === tab.id
        ...
        {canExport && <ExportButton type="profits" label="تصدير الأرباح" />}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-panel p-5 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="text-[#8b92a5] text-sm">الإيرادات المحصّلة</div>
          </div>
          <div className="text-3xl font-bold text-emerald-400">{formatCur(totalRevenue)}</div>
          <div className="text-xs text-[#8b92a5] mt-1">
            {activeTab === 'all' ? `إجمالي ${payments.length} دفعة` : `من ${filteredPayments.length} دفعة في هذه الفترة`}
          </div>
        </div>
        ...
        <div className="glass-panel p-5 relative overflow-hidden">
          ...
          <div className="text-xs text-[#8b92a5] mt-1">
            {activeTab === 'all' ? `إجمالي ${expenses.length} سند صرف` : `من ${filteredExpenses.length} سند صرف في هذه الفترة`}
          </div>
        </div>
        ...
        <div className="glass-panel p-5 relative overflow-hidden border-orange-500/20">
          ...
          <div className="text-xs text-[#8b92a5] mt-1">من إجمالي الحجوزات النشطة</div>
        </div>
      </div>

      {/* الرسم البياني الشهري الحقيقي الثابت */}
